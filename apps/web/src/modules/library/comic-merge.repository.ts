...
export interface ComicMergeRepository {
  ...
  mergeAsChapters(sourceComicIds: string[], targetComicId: string): Promise<ComicMergeResult[]>;
...
export function createComicMergeRepository(): ComicMergeRepository {
  return {
    async mergeAsChapter(sourceComicId, targetComicId) {
      const results = await mergeAsChapters([sourceComicId], targetComicId);
      return results[0]!;
    },
...
async function mergeAsChapters(sourceComicIds: string[], targetComicId: string): Promise<ComicMergeResult[]> {
  bootstrapDatabase();

  const uniqueSourceIds = Array.from(new Set(sourceComicIds.map((id) => id.trim()).filter(Boolean)));
  if (uniqueSourceIds.length === 0) {
    throw new Error("至少选择一本要合并的漫画。");
  }

  if (uniqueSourceIds.includes(targetComicId)) {
  ...
  const sources = uniqueSourceIds.map((sourceComicId) => getComicForMerge(sourceComicId));
  ...
  const sourceChapters = validSources.map((source) => {
    const comicChapters = db
      .select({
        id: chapters.id,
      ...
      .from(chapters)
      .where(eq(chapters.comicId, source.id))
      .orderBy(chapters.sortOrder)
      .all();

    if (comicChapters.length !== 1) {
      throw new Error("MVP 仅支持把单章节漫画合并为目标漫画的一章。");
    }

    return { source, chapter: comicChapters[0]! };
  });
  ...
  const now = new Date().toISOString();
  const results: ComicMergeResult[] = [];

  db.transaction((tx) => {
    for (const { source, chapter } of sourceChapters) {
