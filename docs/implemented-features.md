...
- When OpenList offline submit returns duplicate task (code 10008), the app enqueues the offline task for cloud-library recovery, runs a single-flight paginated index scan of `/115Open/HENTAI/exhentai` (flat `{mangaName}/zip|cbz`, TTL-reusable), then batch-matches pending 10008 tasks to archives and creates transfer tasks. Concurrent 10008s share one scan; miss/ambiguous cases fail with Chinese guidance. Offline worker tick drains pending recovery when a fresh index exists.
