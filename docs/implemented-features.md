...
- 漫画根目录拥有稳定的逻辑 ID，并可为 `windows`、`wsl`、`linux` 分别保存运行位置；当前后台会显示 profile、location 验证状态和根目录离线状态。
- 本地文件以 `manga_root_id + portable relative_path` 作为稳定身份；Windows 与 WSL 位置切换不会重新创建 comic、local_file、chapter 或 page。
...
- 根目录 location 离线或无效时，扫描会失败并保留已有记录，不会把整个根目录下的文件批量标记为缺失；文件维护页会单独显示“根目录离线”。
...
- Reader 页面、封面和缩略图都通过根目录 location resolver 解析当前 profile 的实际文件位置；页面接口仍只接受 `pageId`。
- 支持沉浸模式，隐藏不必要的阅读器工具栏。
- 多章节漫画会在章节切换处显示分隔条。
...
- 压缩包文件列表和页面图像缓存有独立记录。
- 压缩包文件列表和 reader 缩略图缓存键使用逻辑文件/page 身份、文件事实、尺寸和用途，不把跨平台绝对路径作为业务身份；缓存可在迁移后按需重建。
...
- `/admin/paths` 提供跨环境路径迁移预览和应用：迁移前生成只读报告、检查目标目录和活动传输、自动创建 SQLite 备份，并要求显式确认后才在事务中切换 location；该流程不会移动真实漫画文件。
- 迁移报告带 fingerprint 和业务 ID/count invariant 校验；预览过期、存在活动传输、目标离线或路径不可映射时，API 会阻止应用。
...
| `POST` | `/api/admin/path-migration/preview` | 只读生成跨 runtime profile 的路径迁移报告 |
| `POST` | `/api/admin/path-migration/apply` | 在备份、指纹、目标位置和业务 ID 校验通过后应用路径映射 |
...
- 核心配置：`settings`、`manga_roots`、`manga_root_locations`、`scan_sessions`。
