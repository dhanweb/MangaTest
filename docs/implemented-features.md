...
- When OpenList offline submit returns duplicate task (code 10008), the app enqueues the offline task for cloud-library recovery, runs a single-flight paginated index scan of `/115Open/HENTAI/exhentai` (115 common layout `{name}.zip/{name}.zip`, TTL-reusable), then batch-matches pending 10008 tasks to archives and creates transfer tasks. Concurrent 10008s share one scan; miss/ambiguous cases fail with Chinese guidance without reusing the raw 10008 text. Offline worker tick drains pending recovery when a fresh index exists. Admin downloads page can force rescan via `POST /api/downloads/openlist/library-index/rescan` (bypasses TTL, rematches pending + previous index miss/ambiguous tasks).
